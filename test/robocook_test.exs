defmodule RobocookTest do
  ...
  test "make a salad", ctx do
    ...
    {:ok, {:put_item, nil, {:item, :cabbage, 0}}, level} =
    ...
    {:ok, {:pick_item, {:item, :chopped_tomato, 0}, nil}, level} =
      LevelMap.perform_action(level, :pick_up, 0, rules)
    ...
    {:ok, {:put_item, nil, {:container, :plate, {:item, :salad, 0}}}, level} =
      LevelMap.perform_action(level, :put_down, 0, rules)
    ...
    {:ok, {:delivered, {:item, :salad, 0}}, level} =
      LevelMap.perform_action(level, :put_down, 0, rules)
